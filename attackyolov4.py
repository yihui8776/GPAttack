...
def detect(img,net,conf_thresh=0.8, nms_thresh=0.3, use_cuda=1):
    ...
    for output in layerOutputs:
        ...
        for detection in output:
            ...
            if confidence > conf_thresh:
                ...
                # 非极大值抑制
    idxs = cv2.dnn.NMSBoxes(boxes, confidences, 0.2, nms_thresh)  # 置信度阈值和nms阈值为0.2和0.3
...
def create_grid_mask(darknet_model, image_path, lines=3, box_scale=1.0, shape=(416, 416)):
    mask = torch.zeros(*shape, 3)
    ...
    boxes = do_detect(darknet_model, img1, 0.8, 0.3, True)
    h, w = numpy.array(img).shape[:2]
    yolo_boxes = [[(box[0] - box[2] / 2.0) * w, (box[1] - box[3] / 2.0) * h, 
        (box[0] + box[2] / 2.0) * w, (box[1] + box[3] / 2.0) * h] for box in boxes]
    boxes = yolo_boxes
...
def specific_attack(model_helpers, img_path, mask, save_image_dir):
    ...
    while t < max_iterations:
        ...
        #创建一个事件循环，asyncio异步io
        res = loop.run_until_complete(asyncio.gather(*tasks)) #把tasks加入到循环中去，让task异步循环，相当于并发训练
        for al, on in res:
            attack_loss += al
            object_nums += on
